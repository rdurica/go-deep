// Package httpapi je HTTP adaptér nad úložištěm záložek.
package httpapi
...
func NewServerWith(st *store.Store, opts Options, idGen IDGen, now Clock) http.Handler {
	if idGen == nil {
		idGen = randomID
	}
	if now == nil {
...
func (s *Server) search(w http.ResponseWriter, r *http.Request) {
	...
	page, err := s.store.Search(store.Query{
		Text:   q.Get("q"),
		Tag:    q.Get("tag"),
		Limit:  limit,
		Cursor: q.Get("cursor"),
